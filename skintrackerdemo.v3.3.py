    ...
    with sqlite3.connect(DB_PATH) as con:
        ...
        cur.execute("""
                    INSERT INTO records(lesion_id, ts, img_path, itch, pain, sleep, stress, triggers, new_products,
                                        meds_taken, adherence, notes, area_cm2, redness, border_irreg, asymmetry,
                                        depig_deltaE)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
...
def med_schedule_for(lesion_id):
    ...
    with sqlite3.connect(DB_PATH) as con:
        return pd.read_sql_query("SELECT * FROM med_schedule WHERE lesion_id=? ORDER BY id ASC", con,
                                 params=(lesion_id,))
...
def insert_food_log(lesion_id, ts, food_item, category, quantity, meal_type, skin_reaction, reaction_delay_hours,
                    notes):
...
def insert_stress_log(lesion_id, ts, stress_type, stress_level, duration_hours, symptoms, coping_methods, skin_impact,
                      notes):
...
def insert_sun_exposure_log(lesion_id, ts, exposure_type, duration_minutes, time_of_day, uv_index, protection_methods,
                            skin_improvement, side_effects, notes):
...
def insert_medication_log(lesion_id, ts, medication_name, medication_type, dose, frequency, taken_as_prescribed,
...
def view_sun_exposure_patterns():
    ...
    print("-" * 90)
    print(
        f"{'Exposure Type':<20} {'Occurrences':<12} {'Avg Duration':<10} {'Avg Improvement':<12} {'Avg UV Index':<12}")
    print("-" * 90)
...
def view_medication_effectiveness():
    ...
    print("-" * 80)
    print(
        f"{'Medication Name':<20} {'Type':<15} {'Usage Count':<12} {'Avg Effectiveness':<15} {'Avg Adherence':<15} {'Side Effects':<15}")
    print("-" * 80)
    ...
    if not medication_history.empty:
        print(
            f"{'Date':<12} {'Name':<20} {'Type':<15} {'Dose':<10} {'Freq':<10} {'Effectiveness':<15} {'Adherence':<15} {'Notes':<20}")
        print("-" * 60)
...
def display_statistical_summary(lesion_id, days=30):
    ...
    if 'symptoms' in summary:
        ...
        print(
            f"Average Itch Level: {symptoms['itch']['mean']:.2f}/10 (Min: {symptoms['itch']['min']}, Max: {symptoms['itch']['max']})")
        print(
            f"Average Pain Level: {symptoms['pain']['mean']:.2f}/10 (Min: {symptoms['pain']['min']}, Max: {symptoms['pain']['max']})")
        print(
            f"Average Stress Level: {symptoms['stress']['mean']:.2f}/10 (Min: {symptoms['stress']['min']}, Max: {symptoms['stress']['max']})")
        print(
            f"Average Sleep Hours: {symptoms['sleep']['mean']:.2f} hours (Min: {symptoms['sleep']['min']}, Max: {symptoms['sleep']['max']})")
    ...
    if 'food' in summary:
        ...
        print(
            f"Food Reactions: {food['reactions_count']} ({food['reactions_count'] / food['total_entries'] * 100:.1f}% of entries)")
